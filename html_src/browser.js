 ...
 * 核心处理逻辑全部委托给 texture-processor.js
 *
 * 依赖：
 *   1. texture-processor.js（已导出 processTexture 等）
...
(function () {
  ...
  // ========== 状态 ==========
  let currentSourceImage = null;   // 原始图片
  let currentResultCanvas = null;  // 最终结果
  ...
  function applyOutlinePreset(value) {
    // 需要一张临时 32×32 才能算平均色，这里简化：直接用当前源图估算
    // 更精确可在 processTexture 内部再算，这里只做 UI 同步
